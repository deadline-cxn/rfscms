...
function todo_list_action_() { eval(lib_rfs_get_globals());
	...
	echo "<hr>";
	lib_buttons_make_button("$RFS_SITE_URL/modules/todo_list/todo_list.php?action=new_todo_list","New List");
	echo "<hr>";
...
function todo_list_action_view_todo_list($list) { eval(lib_rfs_get_globals());
	...
	lib_buttons_make_button("$RFS_SITE_URL/modules/todo_list/todo_list.php?action=search&tdl=$tdl->id","Search");
	lib_buttons_make_button("$RFS_SITE_URL/modules/todo_list/todo_list.php?action=open_task&tdl=$tdl->id","Open");
...
where (`list`='$tdl->id')  ;");
	...
	if($n>0) {
		...
		while($task=mysql_fetch_object($r)) {
			...
			lib_buttons_make_button("$RFS_SITE_URL/modules/todo_list/todo_list.php?action=edit_task&task=$task->id","Edit");
			echo "</td>";
