...
echo "<script> function playvid(x,y) { document.getElementById(x).innerHTML=\"<iframe src='\"+y+\"' width=400 height=300> </iframe>\"; }
...
if(lib_access_check("files","sort")) {
	...
	if($_SESSION['thumbs']=="true") {
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?thumbs=off&$outvars","Thumbs Off");		
	}
	else {
		echo "HIDE Thumbs<br>";
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?thumbs=on&$outvars","Thumbs On");
	}
	echo "</td>";
	...
	if($_SESSION['hidden']=="yes") {
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?hidden=hide&$outvars","Hidden Off");		
	}
	else {
		echo "HIDE HIDDEN<br>";
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?hidden=show&$outvars","Hidden On");
	}
	echo "</td>";
	...
	if($_SESSION['worksafemode']!="off") {
		echo "WORKSAFE ON<br>";
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?worksafe=off&$outvars","Worksafe off");
	}else {
		echo "<font style='background-color:red;'>WORKSAFE OFF</font><br>";	
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?worksafe=on&$outvars","Worksafe on");		
	}
	echo "</td>";
	...
	if($_SESSION['show_temp']==true){
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?temp=hide&$outvars","Sort Off");
	}
	else {		
		echo "SORT OFF<br>";	
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?temp=show&$outvars","Sort On");
	}
	echo "</td>";
}
if(lib_access_check("files","edit")) {	
	...
	if($_SESSION['editmode']==true){
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?editmode=off&$outvars","Edit Off");		
	}else {
		echo "EDIT OFF<br>";	
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?editmode=on&$outvars","Edit On");		
	}
	echo "</td>";
}
...
if(lib_access_check("files","delete")) {	
	...
	if($_SESSION['deletemode']==true){
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?deletemode=off&$outvars","Delete Off");
	}else {
		echo "DELETE OFF<br>";	
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?deletemode=on&$outvars","Delete On");
	}
	echo "</td>";
}
...
if(lib_access_check("files","edit")) {	
	...
	if($_SESSION['tagmode']==true){
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?tagmode=off&$outvars","Tag Off");
	}else {
		echo "TAG OFF<br>";	
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?tagmode=on&$outvars","Tag On");
	}
	echo "</td>";
}
...
if(lib_access_check("files","sort")) {
	echo "<td>";
	echo "<br>";
	lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=show_duplicates","Show Duplicates");
	echo "</td>";
}
...
if(lib_access_check("files","upload")) {
	echo "<td>";
	echo "<br>";
	lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=upload","Upload");
	echo "</td>";
}
if(lib_access_check("files","addlink")) {
	echo "<td>";    
	echo "<br>";
    lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=addfilelinktodb","Add Link as File");
	echo "</td>";
}
if(lib_access_check("files","orphanscan")) {
	echo "<td>";
	echo "<br>";
    lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=getorphans","Add orphan files");
	echo "</td>";
}
if(lib_access_check("files","purge")) {
	echo "<td>";
	echo "<br>";
	lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=purge","Purge missing files");
	echo "</td>";
}
...
function files_action_addfilelinktlib_mysql_open_database() { eval(lib_rfs_get_globals()); // if($action=="addfilelinktodb") {
    echo "<table border=0>\n";
    echo "<form enctype=application/x-www-form-URLencoded action=$RFS_SITE_URL/modules/core_files/files.php method=post>\n";
...
function files_action_addfilelinktodb_go() { eval(lib_rfs_get_globals());
...
function files_action_addfiletodb_go() { eval(lib_rfs_get_globals());
    $file_url=addslashes($file_url);
    $file_add=addslashes($file_add);
    $description=addslashes($description);
    $name=addslashes($name);
...
function files_action_get_file() { eval(lib_rfs_get_globals());
	if( (lib_rfs_bool_true($RFS_SITE_ALLOW_FREE_DOWNLOADS)) ||
		...
		echo "<a href=\"$RFS_SITE_URL/modules/core_files/files.php?action=get_file_go&id=$filedata->id\" target=_new_window>";
		echo "<font size=4>";
		...
		if(lib_access_check("files","edit")) {
			echo "<td>";
				lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=mdf&id=$filedata->id","Edit");		
			echo "</td>";
		}
		if(lib_access_check("files","delete")) {
			echo "<td>";
			lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=del&id=$filedata->id","Delete");
			echo "</td>";
		}		
		echo "</tr></table>";
...
function files_action_f_dup_rem_checked() { eval(lib_rfs_get_globals());
	if(lib_access_check("files","delete")) {
...
function files_action_ren() { eval(lib_rfs_get_globals()); 
	if(lib_access_check("files","edit")) {
		if(!empty($data->name))    {
...
function files_action_mod() { eval(lib_rfs_get_globals()); // //  if($action=="mod")        {
	if(lib_access_check("files","edit")) {
		...
		echo "<p><a href=$RFS_SITE_URL/modules/core_files/files.php>File</a> modified...</p><br>\n";
	}
	else {
...
///////////////////////////////////////////////////////////////////////////////////
function files_action_getorphans() { eval(lib_rfs_get_globals()); //if($action=="getorphans") {
	orphan_scan("files",0);
	include("footer.php");
    exit();
}
///////////////////////////////////////////////////////////////////////////////////
function files_action_purge() { eval(lib_rfs_get_globals()); // if($action=="purge") {
	purge_files(0);
	include("footer.php");
	exit();	
}
...
function files_action_f_upload_go() { eval(lib_rfs_get_globals());
	lib_rfs_var("fu_dir");
	
	if($fu_dir=="- Select -") { lib_forms_info("You must choose a location","WHITE","RED"); files_action_upload(); exit(); }
	...
	if(empty($data->name)) {
		...
		echo "<p>[<a href=$RFS_SITE_URL/modules/core_files/files.php?action=upload>Add another file</a>]\n";
		echo "[<a href=$RFS_SITE_URL/modules/core_files/files.php>Files</a>]</p>\n";
		}
	}
}
function files_action_upload() { eval(lib_rfs_get_globals()); 
	if(empty($data->name)) {  echo "<p>You must be logged in to upload files...</p>\n"; }
		...
		if(!lib_access_check("files","upload")) {  echo "<p>You are not authorized to upload files!</p>\n";  }
				...
				echo "<form enctype=\"multipart/form-data\" action=\"$RFS_SITE_URL/modules/core_files/files.php\" method=\"post\">\n";
				echo "<table border=0>\n\n\n";
				echo " <!--  ******************************************************************************** --> \n";
				echo "<tr>\n";			
...
function files_action_file_change_category() { eval(lib_rfs_get_globals()); //if($action=="file_change_category") {
	lib_mysql_query("update files set category = '$name' where id = '$id'");
	$name=""; $action="search"; $category="all categories";	
}

function files_action_search() {files_action_listcategory(); }
function files_action_listcategory() { eval(lib_rfs_get_globals());
files_header();
if($_SESSION['show_temp']) {
	...
	lib_forms_info("SORT MODE","WHITE","RED");
		...
		if($prevtop>0) 
			lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=listcategory&amount=$amount&top=$prevtop&category=$category&criteria=$criteria&tagsearch=$tagsearch","PREV PAGE");
		if($x==$amount) 
			lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=listcategory&amount=$amount&top=$nexttop&category=$category&criteria=$criteria&tagsearch=$tagsearch","NEXT PAGE");
		...
		lib_buttons_make_button("$RFS_SITE_URL/modules/core_files/files.php?action=listcategory&$reload&category=$category&criteria=$criteria","RELOAD");
...
function files_action_() {eval(lib_rfs_get_globals());
files_header();
...
if($_SESSION['show_temp']) {
	...
	while($cat=mysql_fetch_object($result)) {		
		if(!empty($cat->name))     {
			...
			if(lib_rfs_bool_true($_SESSION['hidden']))  $shide="";
			...
			if(count($filelist)){				
				...
				if(file_exists($iconp)) {
					echo "<a href=\"$RFS_SITE_URL/modules/core_files/files.php?action=listcategory&category=$buffer\" title=\"List all $buffer files\">";
					echo "<img src=$icon border=0 width=32 height=32>";
					echo "</a>";
				}
				
				echo "<a class=file_category_link href=\"$RFS_SITE_URL/modules/core_files/files.php?action=listcategory&category=$buffer\" title=\"List all $buffer files\">[";			
				echo ucwords("$buffer");
				echo "] ";
