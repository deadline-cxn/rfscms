...
if($action=="list_exams") {
    ...
    if( lib_access_check("exams","edit") ) { 
            echo "[<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_edit'>Exam Administration</a>]";
    }	
    ...
    for($i=0;$i<$r->num_rows;$i++) {
			...
			echo "<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=wipe_exam&exam_id=$exam->id'>
			<img src='$RFS_SITE_URL/images/icons/Play.png' width=16 border=0></a>
			</td>";
			
			echo "<td class=rfs_project_table_$gt>";
			
			echo "<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=wipe_exam&exam_id=$exam->id'>$exam->name</a>";
...
if($action=="wipe_exam") {
	...
	if($oclr=="GREEN") {
		if( $etotq == $etotqa ) {
			if($_SESSION["question_id"]<1) {
				...
				<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=wipe_exam&exam_id=$exam_id&confirmed=true'><img src=$RFS_SITE_URL/images/icons/Play.png border=0>click here</a>. Note: This action can not be reversed.");
				for($i=0;$i<35;$i++) echo "<br>";
				include("footer.php");
				exit();		
	...
	if($x) {
		...
		if(($texam_sequence) > 1) { 
			...
			<center>
			".
			lib_buttons_text("$RFS_SITE_URL/modules/core_exams/exams.php?exam_id=$exam->id&action=really_wipe_exam","RESTART")
			.
			lib_buttons_text("$RFS_SITE_URL/modules/core_exams/exams.php?exam_id=$exam->id&action=run_exam","RESUME")."
			</center>
			<hr>
			<br>
...
if($action=="run_exam") {
	...
	if(empty($question->type)) {
		...
		*/
		...
		$qq.="[<a href=\"$RFS_SITE_URL/modules/core_exams/exams.php?action=wipe_exam&exam_id=$exam_id\">Take this exam again</a>]";
		$qq.="[Review missed questions]";
		$qq.="[Review all questions]";
		
		if( lib_access_check("exam_questions","edit") ) {
				$qq.="[<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_edit'>Exam Administration</a>]";
		}
    ...
    $qq.="<form action=\"$RFS_SITE_URL/modules/core_exams/exams.php\" method=\"post\">\n";
    $ql= " <hr><center>
...
if(lib_access_check("exams","edit") ) {
	...
	if($action=="admin_exam_question_edit") {
		...
		echo "[<a href=$RFS_SITE_URL/modules/core_exams/exams.php?action=run_exam&exam_id=$exam_id>Run this exam</a>]";
		echo "[<a href=$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_edit>List all exams</a>]";
		echo "[<a href=$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_exam_edit&exam_id=$exam_id>List all questions</a>]<br>";
				
		$qt=lib_mysql_fetch_one_object("select * from exam_questions where id='$q'");
			
		lib_forms_build( "$RFS_SITE_URL/modules/core_exams/exams.php",
	...
	if($action=="admin_exam_edit_add") {
		...
		lib_forms_optionize(	"$RFS_SITE_URL/modules/core_exams/exams.php",
    ...
    if($action=="admin_exam_edit") {
		...
		echo "[<a href=$RFS_SITE_URL/modules/core_exams/exams.php?action=run_exam&exam_id=$exam_id>Run this exam</a>]";
		echo "[<a href=$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_edit>List all exams</a>]<br>";
		
		echo "<table border=0><tr><td> Method: $exam->method </td><td>";		
		echo " <form action='$RFS_SITE_URL/modules/core_exams/exams.php' method='post'>	
    ...
    // Matching
        ...
        for($i=0;$i<$r->num_rows;$i++){
			...
			echo "<td class=rfs_project_table_$gt>";
                ...
                ///// edit
                                    echo "<td>";
                echo "<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_exam_edit&exam_id=$exam->id'>";
                ...
                ///// delete
                                    echo "<td>";
                echo "<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_exam_remove&exam_id=$exam->id'>";
                ...
                ///// rename
                                    ...
                                    echo "<td>";
                echo "<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_exam_rename&exam_id=$exam->id'>";
                ...
                ///// run
                                    ...
                                    echo "<td>";
                echo "<a href='$RFS_SITE_URL/modules/core_exams/exams.php?action=admin_exam_test&exam_id=$exam->id'>";
