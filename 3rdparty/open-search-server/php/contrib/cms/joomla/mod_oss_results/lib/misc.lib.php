 ...
 * @param array $curlInfo By Reference. If given, the informations provided by curl will be returned using the provided array
 * @return SimpleXMLElement Will return false if something gone wrong
 */
function retrieveXML($url, &$curlInfo = null) {
...
class NewsFeedParser_Atom extends NewsFeedParser {
	...
	public function __construct(SimpleXMLElement $xml) {
		...
		$this->channelHome		= preg_replace('/(\.\w+)\/.*$/', '$3', (string)$xml->id);
...
function beautifulXML($string) {
	...
	function beautifulXML_tagging($string) {
		$string = preg_replace('/^(\w+)/i', '<span class="nodeName">$3</span>', $string);
		return $string;
	}
